...
const Header = () => {
  ...
  return (
    <nav className='flexBetween padding-container max-container relative z-10 py-5'>
      ...
      { open ? (<div className='absolute md:hidden block bg-red-100 w-[50%] px-10 py-60 right-0 top-24 rounded-lg'>
        <div className='absolute top-12 regular-14 flex flex-col text-white/70 gap-4 text-[18px] hover:text-white hover:transition-200'>
          <div className='py-8'>
          { NAVLINKS.map((nav) => (
                        <Link href={nav.link} key={nav.label} className='bold-20 text-black/80 block cursor-pointer pb-1.5 transition-all hover:font-bold'>
                            {nav.name}
                        </Link>
                    ))}
          </div>
